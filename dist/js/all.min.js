...
var APP={serializeFormJson:function(form){var o={},returnJson={},a=$(form).serializeArray();return $.each(a,function(){o[this.name]?(o[this.name].push||(o[this.name]=[o[this.name]]),o[this.name].push(this.value||"")):o[this.name]=this.value||""}),$.each(o,function(i,item){""!=item.replace(/\s/g,"")&&(returnJson[i]=item)}),returnJson},populateTaskTable:function(table,data){$(table+" tbody").empty(),$.each(data,function(i,item){time_hour=item.time_hour?item.time_hour+"h ":"",time_minute=item.time_minute?item.time_minute+"m ":"",row_duration=time_hour+""+time_minute,$("<tr>").append($("<td>").text(item.description),$("<td>").text(row_duration),$("<td>").html('<button type="button" class="btn btn-secondary btn-sm edit" rel="'+item.id+'"><i class="fa fa-pencil-alt"></i></button>'),$("<td>").html('<button type="button" class="btn btn-danger btn-sm delete"  rel="'+item.id+'"><i class="fa fa-trash-alt"></i></button>')).appendTo(table+" tbody")})},populateForm:function(form,data){$.each(data,function(key,value){var $ctrl=$(form).find("[name="+key+"]");if($ctrl.is("select"))$("option",$ctrl).each(function(){this.value==value&&(this.selected=!0)});else if($ctrl.is("textarea"))$ctrl.val(value);else switch($ctrl.attr("type")){case"text":case"number":case"hidden":$ctrl.val(value);break;case"checkbox":"1"==value?$ctrl.prop("checked",!0):$ctrl.prop("checked",!1)}})},resetForm:function(form){$(form)[0].reset(),$(form).find("input:hidden").val("")},listAllTask:function(){$.ajax({url:"api/task/list",headers:{"task-token":"123456","Content-Type":"application/json"},method:"GET",dataType:"json",success:function(data){APP.populateTaskTable("table.table-task",data)}})}};$(document).ready(function(){$("form.task").on("submit",function(e){e.preventDefault();var formData=APP.serializeFormJson("form.task"),formMethod=formData.id?"PUT":"POST",action=formData.id?"edit/"+formData.id:"save";$.ajax({url:"api/task/"+action,headers:{"task-token":"123456","Content-Type":"application/json"},method:formMethod,dataType:"json",data:JSON.stringify(formData),success:function(data){"success"==data.type&&(swal({position:"top-end",type:"success",text:"Salvo com sucesso!",showConfirmButton:!1,timer:1500}),APP.resetForm("form.task"),APP.listAllTask())}}),event.preventDefault()}),$("form.task").on("reset",function(e){APP.resetForm("form.task")}),$(document).on("click",".edit",function(e){e.preventDefault(),APP.resetForm("form.task");var task_id=$(this).attr("rel");$.ajax({url:"api/task/get/"+task_id,headers:{"task-token":"123456","Content-Type":"application/json"},method:"GET",dataType:"json",success:function(data){APP.populateForm("form.task",data),$('textarea[name="description"]').focus()}})}),$(document).on("click",".delete",function(e){e.preventDefault();var task_id=$(this).attr("rel");$.ajax({url:"api/task/delete/"+task_id,headers:{"task-token":"123456","Content-Type":"application/json"},method:"DELETE",dataType:"json",success:function(data){"success"==data.type&&(swal({position:"top-end",type:"success",text:"Deletado com sucesso!",showConfirmButton:!1,timer:1500}),APP.listAllTask())}})}),APP.listAllTask(),$('[data-toggle="popover"]').popover()});
/*!
